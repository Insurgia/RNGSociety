...
function ScannerTab() {
  ...
  const verifyAgainstDb = (ai) => {
    ...
    const norm = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9ã€-ãƒ¿ã€-é¿¿]/g, '')
    ...
    <div className="scanner-grid">
      ...
      <div className="panel">
        ...
        {aiResult ? <div className="ai-result">
          ...
          <div className="muted">Native: {aiResult.card_name_native || '-'} ï¿½ EN: {aiResult.card_name_english || '-'}</div>
          <div className="muted">Set native: {aiResult.set_name_native || aiResult.set_name || '-'} ï¿½ Set EN: {aiResult.set_name_english || '-'}</div>
          <div className="muted">No: {aiResult.card_number || '-'} ï¿½ Rarity: {aiResult.rarity || '-'} ï¿½ Detected lang: {aiResult.detected_language || '-'}</div>
